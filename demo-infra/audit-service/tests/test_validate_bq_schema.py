...
def test_matching_schemas_pass(make_schema_file):
    ...
    result = subprocess.run(["python3", "../scripts/validate_bq_schema.py", str(emitted), str(bq)])
    assert result.returncode == 0

def test_mismatched_schemas_fail(make_schema_file):
    emitted = make_schema_file("emitted.json", '{"fields": [{"name": "user_agent", "type": "STRING", "mode": "NULLABLE"}, {"name": "new_field", "type": "STRING", "mode": "NULLABLE"}]}')
    bq = make_schema_file("bq.json", '{"fields": [{"name": "user_agent", "type": "STRING", "mode": "NULLABLE"}]}')
    result = subprocess.run(["python3", "../scripts/validate_bq_schema.py", str(emitted), str(bq)])
